...
describe('AuthRegisterController', () => {
  ...
  beforeEach(() => {
    ...
    authRegisterController = new AuthRegisterController(mockAuthRegisterUseCase)
  })

  it('should call authRegisterUsecase with correct parameters', async () => {
    const authResult = 'User created successfully'

    jest.spyOn(mockAuthRegisterUseCase, 'authRegister').mockResolvedValueOnce(authResult)

    await authRegisterController.handle(mockRequest)

    expect(mockAuthRegisterUseCase.authRegister).toHaveBeenCalledWith(requestBody)
  })
  ...
  it('should return 409 if registration fails due to duplicate email', async () => {
    ...
    jest.spyOn(mockAuthRegisterUseCase, 'authRegister').mockResolvedValueOnce(errorResult)
  ...
  it('should return 500 if an unexpected error occurs', async () => {
    jest.spyOn(mockAuthRegisterUseCase, 'authRegister').mockRejectedValueOnce(new Error('Internal server error'))
