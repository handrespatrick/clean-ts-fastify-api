...
export class UpdateCategoryService implements IUpdateCategoryUseCase {
  constructor(private readonly _categoryRepository: IUpdateCategoryRepository) {}

  async update(data: IUpdateCategoryUseCase.Params): Promise<IUpdateCategoryUseCase.Result> {
    const { id } = data
